...
const PaqueteUpdate = ({ paquete, paquetes, productos, setEditPaquete, categorias, token }) => {
    ...
    const [productosSeleccionados, setProductosSeleccionados] = useState(paquete.productos);

    const cambioNombre = (event) => {
        const value = event.target.value;
        setNombre(value);
        const existe = paquetes.some(p => p.nombre.toLowerCase() === value.toLowerCase() && p._id !== paquete._id);
    ...
    const handleSubmit = async (e) => {
        ...
        if (ErrorNombre === '' && ErrorDescripcion === '' && ErrorPrecio === '' && ErrorCategoria === '') {
            setLoading(true);
            const paqueteModificado = { nombre, descripcion, estado, precio, categoria, productos: productosSeleccionados.map(producto => ({ producto: producto.producto._id, cantidad: producto.cantidad })) };
            console.log(paqueteModificado);
            await fetch(url + paquete._id, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'x-token': token
                },
                body: JSON.stringify(paqueteModificado)
            })
            .then(res => res.json())
            .then(data => {
                setLoading(false);
    ...
    return (
        <Form style={{ maxWidth: "800px" }} className='m-auto' onSubmit={handleSubmit}>
            <h3 className='text-center my-3'>Modificar Paquete</h3>
            ...
            <div className='d-flex justify-content-end'>
                {loading ? (
                    ...
                    <>
                        ...
                        <Button variant="danger" type="submit" onClick={() => setEditPaquete({})}>
                            Cancelar
                        </Button>
                    </>
                )}
